...
with tf.device("/gpu:0"):
    ...
    K = 90  # first convolutional layer output depth
    L = 180  # second convolutional layer output depth
    M = 360  # third convolutional layer
    ...
    W5 = tf.Variable(tf.truncated_normal([N, cats], stddev=0.1))
    ...
    YY = tf.reshape(Y3, shape=[-1, 4 * 4 * M])
    ...
    # I = tensorflowvisu.tf_format_mode_decision_images(X, Y, Y_)
    # It = tensorflowvisu.tf_format_mode_decision_images(X, Y, Y_, 1000, lines=25)
    # datavis = tensorflowvisu.mode_decisionDataVis(title4="batch-max conv activation", title5="batch-max dense activations", histogram4colornum=2, histogram5colornum=2)
